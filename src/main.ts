      ...
      editorCallback: async (editor: Editor) => {
        ...
        new PromptModal(
          ...
          async (x: { [key: string]: string }) => {
            ...
            const selectedCritiqueModel =
              x["selectedCritiqueModel"] || this.settings.critiqueModelName;
            const responseLength = x["responseLength"] || "normal";

            // Get the appropriate assistant based on selected model
            const primaryAssistant = this.getAssistantForModel(selectedModel, responseLength);
            ...
            if (critiqueEnabled && answer) {
              ...
              new Notice(
                "Preparing critique... This will take about 60 seconds.",
              );

              // Wait 60 seconds, then call critique model
              setTimeout(async () => {
                try {
                  const critiqueAssistant = this.getAssistantForModel(
                    selectedCritiqueModel,
                    responseLength,
                  );
...
Critique:`;
  ...
  getAssistantForModel(
    modelName: string,
    responseLength: string = "normal",
  ): OpenAIAssistant | AnthropicAssistant | GeminiAssistant | GroqAssistant {
    ...
    if (modelName.startsWith("claude")) {
      ...
      // Calculate max tokens based on response length
      let maxTokens;
      ...
      console.log("🔧 Using response length:", responseLength, "with max tokens:", maxTokens);
...
class AiAssistantSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Select default model for critique mode (should be stronger than primary model)",
      )
      .addDropdown((dropdown) =>
        dropdown
          .addOptions(ALL_MODELS)
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "When ON: Replace selected text with AI response. When OFF: Keep selected text and add AI response below it.",
      )
      .addToggle((toogle) => {
        toogle
